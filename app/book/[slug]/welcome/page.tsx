      ...
      <div className="max-w-md mx-auto">
        ...
        {step === "auth" && (
          <form onSubmit={handleAuth} className="step-enter bg-white border border-black/10 rounded-2xl p-6 space-y-3">
            <p className="text-sm font-semibold text-[#14261F] mb-1">Create your account</p>
            <div className="flex gap-2 mb-2">
              <button
                type="button"
                onClick={() => setAuthMode("signup")}
                className={`flex-1 text-xs rounded-lg py-2 border ${authMode === "signup" ? "bg-[#14261F] text-[#FAF6EF] border-[#14261F]" : "border-black/15 text-[#14261F]"}`}
              >
                New here
              </button>
              <button
            ...
            <input type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#14261F]" />
        ...
        {step === "details" && (
          <form onSubmit={handleSaveDetails} className="step-enter bg-white border border-black/10 rounded-2xl p-6 space-y-3">
            <p className="text-sm font-semibold text-[#14261F] mb-1">A little about you</p>
            <input type="text" required value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="Your name" className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#14261F]" />
            <input type="tel" value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} placeholder="Phone number" className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#14261F]" />
            <button type="submit" disabled={saving} className="w-full bg-[#14261F] text-[#FAF6EF] rounded-full py-2.5 text-sm font-semibold disabled:opacity-50">
              {saving ? "…" : "Continue"}
            </button>
          </form>
        ...
        {step === "style" && (
          <div className="step-enter bg-white border border-black/10 rounded-2xl p-6 space-y-3">
            <p className="text-sm font-semibold text-[#14261F] mb-1">Look and style</p>
            ...
            <input type="text" value={coatType} onChange={(e) => setCoatType(e.target.value)} placeholder="Coat type (e.g. Short, Long, Double)" className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#14261F]" />
        ...
        {step === "personality" && (
          <div className="step-enter bg-white border border-black/10 rounded-2xl p-6 space-y-3">
            <p className="text-sm font-semibold text-[#14261F] mb-1">What&apos;s {dogName || "your dog"} like?</p>
            ...
            <textarea value={about} onChange={(e) => setAbout(e.target.value)} placeholder={`Tell us more about ${dogName || "them"}...`} rows={3} className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#14261F]" />
            <div className="flex gap-2">
              <button onClick={goBack} className="flex-1 border border-black/15 text-[#14261F] rounded-full py-2.5 text-sm font-semibold">Back</button>
              <button onClick={goNext} className="flex-1 bg-[#14261F] text-[#FAF6EF] rounded-full py-2.5 text-sm font-semibold">Continue</button>
            </div>
        ...
        {step === "health" && (
          <div className="step-enter bg-white border border-black/10 rounded-2xl p-6 space-y-3">
            <p className="text-sm font-semibold text-[#14261F] mb-1">Health and care</p>
            ...
            <input type="text" value={medications} onChange={(e) => setMedications(e.target.value)} placeholder="Medications (if any)" className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#14261F]" />
            <div className="flex gap-2">
            ...
            <div className="flex items-center gap-4 text-sm text-[#14261F]">
        ...
        {step === "preferences" && (
          <div className="step-enter bg-white border border-black/10 rounded-2xl p-6 space-y-3">
            <p className="text-sm font-semibold text-[#14261F] mb-1">A few favorites</p>
            ...
            <textarea value={dislikes} onChange={(e) => setDislikes(e.target.value)} placeholder="Things that stress them out (e.g. the dryer, nail trims)" rows={2} className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#14261F]" />
            ...
            <div className="flex gap-2">
              <button onClick={goBack} className="flex-1 border border-black/15 text-[#14261F] rounded-full py-2.5 text-sm font-semibold">Back</button>
              <button onClick={handleFinish} disabled={saving} className="flex-1 bg-[#14261F] text-[#FAF6EF] rounded-full py-2.5 text-sm font-semibold disabled:opacity-50">

...
export default function DogProfilePage({ params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  const load = useCallback(async () => {
    ...
    if (!isNew) {
      ...
      if (dog) {
        setName(dog.name ?? "");
        setSpecies((dog.species as "dog" | "cat" | "other") ?? "dog");
  ...
  return (
    <div className="min-h-screen px-4 py-10 pb-28">
      <div className="max-w-md mx-auto">
        <a href={`/book/${slug}/account`} className="text-xs text-[#1A1A1A]/50 hover:underline">
          ← Back to account
        </a>
        <h1 className="text-xl font-semibold text-[#1A1A1A] mt-2 mb-6">
        ...
        <div className="space-y-4">
          <div className="bg-white border border-black/10 rounded-2xl p-5 space-y-3">
            <p className="text-xs font-semibold text-[#1A1A1A]/50 uppercase tracking-wide">Basics</p>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Their name" className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#1A1A1A]" />
          ...
          <div className="bg-white border border-black/10 rounded-2xl p-5 space-y-3">
            <p className="text-xs font-semibold text-[#1A1A1A]/50 uppercase tracking-wide">Look &amp; style</p>
            ...
            <input type="text" value={coatType} onChange={(e) => setCoatType(e.target.value)} placeholder="Coat type" className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#1A1A1A]" />
          ...
          <div className="bg-white border border-black/10 rounded-2xl p-5 space-y-3">
            <p className="text-xs font-semibold text-[#1A1A1A]/50 uppercase tracking-wide">Personality</p>
            ...
            <textarea value={about} onChange={(e) => setAbout(e.target.value)} placeholder={`Tell us more about ${name || "them"}...`} rows={3} className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#1A1A1A]" />
          </div>

          <div className="bg-white border border-black/10 rounded-2xl p-5 space-y-3">
            <p className="text-xs font-semibold text-[#1A1A1A]/50 uppercase tracking-wide">Health &amp; care</p>
            ...
            <input type="text" value={medications} onChange={(e) => setMedications(e.target.value)} placeholder="Medications" className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#1A1A1A]" />
            <div className="flex gap-2">
          ...
          <div className="bg-white border border-black/10 rounded-2xl p-5 space-y-3">
            <p className="text-xs font-semibold text-[#1A1A1A]/50 uppercase tracking-wide">Favorites &amp; quirks</p>
            ...
            <textarea value={dislikes} onChange={(e) => setDislikes(e.target.value)} placeholder="Things that stress them out" rows={2} className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-[#1A1A1A]" />
